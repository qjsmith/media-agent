...
def build_tv_path(
    ...
    filename = sanitize(f"{show_title} ({show_year}) - S{season:02d}E{episode:02d} - {episode_title}{extension}")
    return str(Path(base_path) / show_folder / season_folder / filename)


def build_movie_path(base_path: str, movie_title: str, movie_year: int, extension: str) -> str:
